            ...
            <div class="ibox-content">
                <div class="row text-center">
                  <?php $total = 0; foreach ($emballages as $key => $emballage) {
                    $stock = $produit->enBoutique(Home\PARAMS::DATE_DEFAULT, dateAjoute(1), $emballage->id, $boutique->id);  ?>
                        ...
                        <tbody>
                            ...
                            while ($index <= $date2) { ?>
                                <tr>
                                    ...
                                    <?php foreach ($emballages as $key => $emballage) {
                                        $stock = $produit->enBoutique(Home\PARAMS::DATE_DEFAULT, $index, $emballage->id, $boutique->id);
                                        $appro = $produit->totalMiseEnBoutique($index, $index, $emballage->id, $boutique->id);
                                        $vendu = $produit->vendu($index, $index, $emballage->id, $boutique->id);
                                        $livre = $produit->livree($index, $index, $emballage->id, $boutique->id);
                                        $conversion = $produit->transfertboutique($index, $index, $emballage->id, $boutique->id);
                                        $transfert = $produit->transfertboutiqueEntrant($index, $index, $emballage->id, $boutique->id) - $produit->transfertboutiqueSortant($index, $index, $emballage->id, $boutique->id);
                                        $perte = $produit->perteBoutique($index, $index, $emballage->id, $boutique->id) + $produit->perteProspection($index, $index, $emballage->id, $boutique->id);
                                        ?>
                                        ...
                                        data-html="true"
                                        data-content="
                                        <span>Dépôt du jour : <b><?= round($appro, 2) ?> </b></span><br>
                                        <span>Vente du jour : <b><?= round($vendu, 2) ?> </b></span><br>
                                        <span>Livraison du jour : <b><?= round($livre, 2) ?> </b></span><br>
                                        <span>Transfert boutique : <b><?= $transfert ?> </b></span><br>
