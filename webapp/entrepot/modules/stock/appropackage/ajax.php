	...
	if (!in_array($id, $ressources)) {
		...
		if (count($datas) == 1) {
			...
			<tr class="border-0 border-bottom " id="ligne<?= $id ?>" data-id="<?= $id ?>">
				...
				<td class="gras"><br><br><?= $item->unite ?></td>
				<td width="120">
									...
									if ($data->status) {
										...
										if ($data->status) {
											foreach ($ressources as $key => $value) {
												...
												if (count($datas) == 1) {
													$ressource = $datas[0];
													$lignecommande = new LIGNEAPPROPACKAGE;
													$lignecommande->appropackage_id = $approvisionnement->id;
													$lignecommande->package_id = $id;
											...
											if ($modepayement_id != MODEPAYEMENT::PRELEVEMENT_ACOMPTE && $total > 0) {
												...
												$payement->appropackage_id = $approvisionnement->id;
												$data = $payement->save();
											}
											...
											$data->setUrl("fiches", "master", "bonappropackage", $approvisionnement->id);
										}
									}
								}
...
if ($action == "validerAppro") {
	$id = getSession("appropackage_id");
	$datas = APPROPACKAGE::findBy(["id ="=>$id]);
	if (count($datas) > 0) {
		$appro = $datas[0];
		...
		if (count($ressources) > 0) {
			...
			foreach ($tests as $key => $value) {
				...
				$id = $lot[0];
				$qte = end($lot);
				foreach ($appro->ligneappropackages as $key => $lgn) {
					if (($lgn->package_id == $id) && ($lgn->quantite >= $qte)) {
						unset($tests[$key]);
					}
				}
			}
