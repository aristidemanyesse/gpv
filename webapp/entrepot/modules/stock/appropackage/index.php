    ...
    <div id="wrapper">
        ...
        <div id="page-wrapper" class="gray-bg">
            ...
            <div class="row wrapper border-bottom white-bg page-heading">
                <div class="col-sm-8">
                    <h2 class="text-uppercase text-blue  gras">Approvisionnements des extras </h2>
                </div>
                <div class="col-sm-4">
                  <button style="margin-top: 5%" data-toggle='modal' data-target="#modal-appropackage" class="btn btn-success dim"><i class="fa fa-plus"></i> Approvisionnement d'emballage</button>
              </div>
          </div>
                    ...
                    <table class="footable table table-stripped toggle-arrow-tiny">
                        ...
                        <tbody>
                            <?php foreach ($encours as $key => $appro) {
                                $appro->actualise(); 
                                $lots = $appro->fourni("ligneappropackage");
                                ?>
                                <tr style="border-bottom: 2px solid black">
                                    ...
                                    <td class="border-right">
                                        <table class="table table-bordered">
                                            <thead>
                                                <tr class="no">
                                                    <?php foreach ($appro->ligneappropackages as $key => $ligne) {
                                                        $ligne->actualise(); ?>
                                                        <th class="text-center gras"><span class="small"><?= $ligne->package->name() ?></span></th>
                                                    <?php } ?>
                                                </tr>
                                            </thead>
                                            <tbody>
                                                <tr>
                                                    <?php foreach ($lots as $key => $ligne) { ?>
                                                        <td class="text-center"><?= start0($ligne->quantite) ?> <?= $ligne->package->unite ?></td>
                                                    <?php } ?>
                                                </tr>
                                            </tbody>  
                                        </table>
                                    ...
                                    <td>
                                        <a href="<?= $this->url("fiches", "master", "bonappropackage", $appro->id) ?>" target="_blank" class="btn btn-white btn-sm"><i class="fa fa-file-text text-blue"></i></a>
                                        ...
                                        <?php if ($employe->isAutoriser("modifier-supprimer") && $appro->etat_id != Home\ETAT::ANNULEE) { ?>
                                            <button onclick="annuler('appropackage', <?= $appro->id ?>)" class="btn btn-white btn-sm"><i class="fa fa-trash text-red"></i></button>
                                        <?php } ?>
                                    </td>
                                </tr>
                            <?php  } ?>
...
foreach ($encours as $key => $appro) {
    include($this->rootPath("composants/assets/modals/modal-appropackage2.php"));
} 
?>
