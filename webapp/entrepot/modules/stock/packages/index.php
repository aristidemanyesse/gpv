...
<body class="fixed-sidebar">
        ...
        <div id="page-wrapper" class="gray-bg">
          ...
          <div class="ibox">
            <div class="ibox-title">
                <h5 class="text-uppercase">Stock des extras d'emballages</h5>
                <div class="ibox-tools">
                    <button data-toggle='modal' data-target="#modal-appropackage" style="margin-top: -2%" class="btn btn-success btn-xs dim"><i class="fa fa-plus"></i> Nouvel Approvisionnement</button>
            ...
            <div class="ibox-content">
                <div class="row text-center">
                  <?php $total = 0; foreach ($extras as $key => $extra) {
                    $stock = $extra->stock(Home\PARAMS::DATE_DEFAULT, dateAjoute(1), $entrepot->id);
                    $prix = $stock * $extra->price();
                    $total += $prix ?>
                    <div class="col-sm-4 col-md-3 col-lg-2 border-left border-bottom">
                        <div class="p-xs">
                            <i class="fa fa-cubes fa-2x"></i>
                            <h5 class="m-xs gras <?= ($stock > $extra->stkAlert)?"":"clignote" ?>"><?= round($stock, 2) ?> </h5>
                            <h6 class="no-margins text-uppercase gras <?= ($stock > $extra->stkAlert)?"":"clignote" ?>"><?= $extra->name() ?> </h6>
                        ...
                        <thead>
                            <tr>
                                ...
                                <?php foreach ($extras as $key => $extra) {  ?>
                                    <th><small class="gras"><?= $extra->name() ?></small></th>
                                <?php } ?>
                            </tr>
                        </thead>
                        <tbody>
                            ...
                            while ($index <= $date2) { ?>
                                <tr>
                                    ...
                                    <?php foreach ($extras as $key => $extra) {
                                        $stock = $extra->stock(Home\PARAMS::DATE_DEFAULT, $index, $entrepot->id);
                                        $appro = $extra->achat($index, $index, $entrepot->id);
                                        $conso = $extra->consommee($index, $index, $entrepot->id);
                                        $perte = $extra->perte($index, $index, $entrepot->id);
                                        ?>
                                        <td class="cursor myPopover"
                                        data-toggle="popover"
                                        data-placement="left"
                                        title="<small><b><?= $extra->name() ?></b> | <?= datecourt($index) ?></small>"
                                        data-trigger="hover"
                                        data-html="true"
                                        data-content="
    ...
    <?php include($this->rootPath("composants/assets/modals/modal-appropackage.php")); ?>  
...
<script type="text/javascript" src="<?= $this->relativePath("../appropackage/script.js") ?>"></script>
        ...
        <div class="modal-content">
            ...
            <form method="POST" class="formShamman" classname="perteentrepot">
                <div class="modal-body">
                    <div class="row">
                        <div class="col-sm-8">
                            <label>Extra d'emballage perdu <span1>*</span1></label>
